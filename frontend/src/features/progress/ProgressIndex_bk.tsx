...
const ProgressIndex: React.FC<Props> = (props) => {
  ...
  const handleChangeAgeName = (event: React.ChangeEvent<{ value: unknown }>) => {
  ...
  const [state] = useState({
    columns: [
      ...
      { title: "請求書有無", field: "needs_billing", editable: 'never' },
      { title: "結果送付先", field: "result_destination", editable: 'never' },
      { title: "備考", field: "remarks", editable: 'onUpdate' },
      { title: "保険種別", field: "insurance_type", editable: 'never' },
      ...
      .then((response) => {
        ...
        response.data.forEach((el) => {
          data.push({
            ...
            needs_billing: NeedsBillingSelect(el.needs_billing),
